...
from check_tracks import Checktracks
...
class ViewTracks:
    ...
    def check_tracks(self):
        Checktracks(tk.Toplevel(self.root))
    ...
    def create_track_list(self):
        ...
        track_window.title("Track List")
        track_window.geometry("400x300")

        scrolled_text = tkst.ScrolledText(track_window, wrap=tk.WORD, width=40, height=15)
        scrolled_text.pack(pady=10)

    # Populate the scrolled text with each LibraryItem's information
        for item in self.library_items:
            scrolled_text.insert(tk.END, f"{item.info()}\n")
...
check_videos_btn = tk.Button(window, text="Check Tracks", command=lambda: Checktracks(tk.Toplevel(window)))
